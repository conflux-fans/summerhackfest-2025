...
import { ChainController, ConnectorController, ApiController as ControllersApiController, CoreHelperUtil, ModalController, RouterController } from '@reown/appkit-controllers';
...
const NAMESPACES = ['eip155', 'solana', 'bip122'];
describe('AppKitWalletButton', () => {
    beforeAll(() => {
        ...
        vi.spyOn(ControllersApiController, 'prefetch').mockImplementation(() => Promise.resolve([]));
    });
    beforeEach(() => {
    ...
    test('should call connectSocial with correct namespace (eip155, solana and bip122)', async () => {
        for (const namespace of NAMESPACES) {
            ...
            const element = await fixture(html `<appkit-wallet-button wallet="google" namespace="${namespace}"></appkit-wallet-button>`);
            const walletButtonSocial = HelpersUtil.getByTestId(element, WALLET_BUTTON_SOCIAL);
            await walletButtonSocial.click();
            expect(connectSocialSpy).toHaveBeenCalledWith(expect.objectContaining({
                social: 'google',
                namespace
            }));
        }
    ...
    test('should call connectExternal with connector having correct namespace (eip155, solana and bip122)', async () => {
        for (const namespace of NAMESPACES) {
            ...
            const element = await fixture(html `<appkit-wallet-button
          wallet="metamask"
          namespace="${namespace}"
        ></appkit-wallet-button>`);
            const walletButtonExternal = HelpersUtil.getByTestId(element, WALLET_BUTTON_EXTERNAL);
            await walletButtonExternal.click();
            expect(connectExternalSpy).toHaveBeenCalledWith(expect.objectContaining({
                id: 'metamask',
                chain: namespace
            }));
        }
    ...
    test('should call connectWalletConnect with connector having correct namespace (eip155, solana and bip122)', async () => {
        ...
        ></appkit-wallet-button>`);
            const walletButton = HelpersUtil.getByTestId(element, WALLET_BUTTON);
            await walletButton.click();
            expect(connectWalletConnectSpy).toHaveBeenCalledWith(expect.objectContaining({
