...
// USDT contract address on Conflux chain
const CONFLUX_USDT_ADDRESS = '0xfe97e85d13abd9c1c33384e796f10b73905637ce';
...
async function approveToken(provider, privateKey, amount) {
  try {
    ...
    const usdtContract = new ethers.Contract(CONFLUX_USDT_ADDRESS, USDT_ABI, wallet);
...
async function main() {
  try {
    ...
    const provider = new ethers.JsonRpcProvider('https://main.confluxrpc.com');
    ...
    if (config.environment.from.includes('usdt')) {
      console.log('\n=== IMPORTANT NOTE ===');
      console.log('Before proceeding with cross-chain transfer, ensure you have approved Meson contract for USDT token through your Conflux wallet:');
    ...
    if (actualSwapResult.txHash) {
      // Conflux blockchain explorer URL
      const explorerUrl = `https://confluxscan.io/transaction/${actualSwapResult.txHash}`;
