...
export async function bridgeNFT(
  ...
  try {
    ...
    const expectedChainId = direction === 'toBase' ? CONFLUX_CHAIN_ID : BASE_CHAIN_ID;
    const dstEid = direction === 'toBase' ? BASE_EID : CONFLUX_EID;
    const bridgeAddress = direction === 'toBase' ? CONFLUX_BRIDGE_ADDRESS : BASE_BRIDGE_ADDRESS;
    const expectedPeer = direction === 'toBase' ? BASE_BRIDGE_ADDRESS : CONFLUX_BRIDGE_ADDRESS;

    if (chainId !== expectedChainId) {
      setTxStatus(`Switch to ${direction === 'toBase' ? 'Conflux' : 'Base'}`);
      return { status: "failed", txHash: null };
    }
    ...
    const gas = BigInt(300000 * tokenIds.length); // Scale with batch size
